...
namespace coalgasOS
{
    public partial class FormDelCoalgas : FormClass
    {

        public FormDelCoalgas()
        {
            InitializeComponent();
        }
        ...
        private void initMyDataGridView()
        {
            ...
            String sql1 = "select * from coalgas";
            ...
            dataAdapter.Fill(myDataSet, "coalgas");

            //使用DataSet绑定时，必须同时指明DateMember 
            this.dataGridView.DataSource = myDataSet;
            this.dataGridView.DataMember = "coalgas";
        ...
        private void FormDelCoalgas_Load(object sender, EventArgs e)
        {
            initMyDataGridView();
        }

        private void buttonDelCoalgasDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView.SelectedRows)
                {
                    ...
                    //删除
                    string sql = "delete from coalgas where coalgas_id = '" + row.Cells[0].Value.ToString() + "';";
        ...
        private void dataGridView_CellValueChanged(object sender, System.Windows.Forms.DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                double intoPay = (double)dataGridView.Rows[rowIndex].Cells[2].Value;
                double outPay = (double)dataGridView.Rows[rowIndex].Cells[3].Value;
                int num = (int)dataGridView.Rows[rowIndex].Cells[4].Value;
                string note;

                if (dataGridView.Rows[rowIndex].Cells[5].Value == System.DBNull.Value)
                {
                    note = " ";
                }
                else
                {
                    note = (string)dataGridView.Rows[rowIndex].Cells[5].Value;
                }
                ...
                //修改
                string sql = "update coalgas set coalgas_name='" + name + "',coalgas_into_pay=" + intoPay + ",coalgas_out_pay=" + outPay + ",coalgas_num=" + num + ",coalgas_note='" + note + "' where coalgas_id='" + id + "';";
                SqlCommand command = new SqlCommand(sql, connection);
                int isok = command.ExecuteNonQuery();
